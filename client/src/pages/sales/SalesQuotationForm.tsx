    ...
    return (
        <div className="space-y-6 animate-fade-in w-full max-w-[1500px] mx-auto">
            <div className="flex items-center gap-4">
                <button onClick={() => navigate('/sales/quotations')} className="p-2 hover:bg-gray-100 rounded-full transition-colors">
                    <ArrowLeft size={20} />
                </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Quotation Items</h3>
                                <p className="text-xs text-gray-500">Add products and set offer pricing</p>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    type="button"
                                    onClick={() => {
                                        const newItem: QuotationItem = { productId: '', description: '', unitCode: 'UNIT', qty: 1, unitPrice: 0, discount: 0, taxAmount: 0 };
                ...
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm sticky top-6">
                        <h2 className="text-lg font-bold text-gray-900 mb-6 font-mono uppercase tracking-tighter">Quotation Summary</h2>
                        <div className="space-y-4">
                            <div className="flex justify-between text-sm"><span className="text-gray-500">Subtotal</span><span className="font-medium">{totals.subtotal.toLocaleString(undefined, { minimumFractionDigits: 2 })} {currency}</span></div>
                            <div className="flex justify-between text-sm"><span className="text-gray-500">Tax Total</span><span className="font-medium">{totals.taxTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })} {currency}</span></div>
                            <div className="pt-4 border-t border-dashed flex justify-between items-center"><span className="font-bold text-gray-900">Grand Total</span><span className="text-xl font-black text-blue-600">{totals.grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2 })} {currency}</span></div>
                        </div>
                        <button onClick={handleSubmit} disabled={createMut.isPending || items.length === 0 || (!customerId && !customerName.trim())} className="w-full mt-8 flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 shadow-lg shadow-blue-200">
                            <Save size={18} /> {createMut.isPending ? 'Saving...' : 'Save Quotation'}
                        </button>
                    </div>
                </div>
            </div>

            <Modal isOpen={editingIndex !== null && !!editForm} onClose={cancelEditItem} title="Edit Item" maxWidth="md">
                {editForm && (
                    <div className="space-y-4">
                        <div>
                        ...
                        <div className="rounded-lg bg-gray-50 border border-gray-100 px-3 py-2 text-sm flex justify-between">
                            ...
                            <span className="font-semibold text-gray-900">{(Number(editForm.qty || 0) * Number(editForm.unitPrice || 0) - Number(editForm.discount || 0)).toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                        </div>
