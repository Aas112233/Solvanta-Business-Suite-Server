...
export default function StockOnDateReport() {
    ...
    const { data: stockData, isLoading } = useQuery({
        ...
        queryFn: () =>
            api
                ...
                .then((r) => r.data.data as StockOnDateData),
        enabled: !!targetDate,
    });
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            await exportExcel({
                fileName: `stock-on-date-${targetDate}.xlsx`,
                sheetName: 'Stock On Date',
                title: `Inventory Stock On Date (${targetDate})`,
                filters: {
                    'Date': targetDate,
                    'Branch': branchName,
    ...
    return (
        <PageTemplate
            title="Stock on Date Report"
            subtitle="Historical stock snapshot for a selected date with item filters."
            breadcrumb={[
                { label: 'Home', href: '/' },
                { label: 'Reports', href: '/reports' },
                { label: 'Stock on Date Report' },
            ]}
            action={
                <Button
                    variant="primary"
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-border">
                                {previewRows.length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-text-tertiary">No stock found for selected filters.</td></tr>}
                                {previewRows.map((row) => {
                                    const matchedUnit = row.product?.units?.find((u) => String(u.unitCode || '').toLowerCase() === String(row.unitCode || '').toLowerCase());
                                    return (
                                        <tr key={row.id} className="hover:bg-background-subtle transition-colors">
                                            ...
                                            <td className="px-4 py-3 text-text-secondary">{row.product?.name || '-'}</td>
