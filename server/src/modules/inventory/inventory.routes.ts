...
// GET /inventory/stock
// Stock Overview
inventoryRoutes.get('/stock', requirePermission(PERMISSIONS.INVENTORY_VIEW as any), validate({ query: stockListQuerySchema }), asyncHandler(async (req, res) => {
...
inventoryRoutes.get('/alerts', requirePermission(PERMISSIONS.INVENTORY_VIEW as any), validate({ query: inventoryAlertsQuerySchema }), asyncHandler(async (req, res) => {
...
inventoryRoutes.post('/adjust', requirePermission(PERMISSIONS.INVENTORY_ADJUST as any), validate({ body: inventoryAdjustmentSchema }), asyncHandler(async (req, res) => {
...
inventoryRoutes.get('/movements', requirePermission(PERMISSIONS.INVENTORY_VIEW as any), validate({ query: movementsQuerySchema }), asyncHandler(async (req, res) => {
    ...
    // Compute running balance efficiently: instead of N+1 aggregate per movement,
    // fetch current stock for each unique (productId, branchId, unitCode) once,
    // then walk movements in chronological order (oldest first) to compute running balance.
    const uniqueStockKeys = new Map<string, { productId: string; branchId: string; unitCode: string }>();
    for (const m of movements) {
        const key = `${m.productId}|${m.branchId}|${m.unitCode}`;
        if (!uniqueStockKeys.has(key)) {
            uniqueStockKeys.set(key, { productId: m.productId, branchId: m.branchId, unitCode: m.unitCode });
        }
    }

    const currentStocks = await Promise.all(
        [...uniqueStockKeys.values()].map(({ productId, branchId, unitCode }) =>
            prisma.inventoryStock.findFirst({
                where: { companyId: req.user!.companyId, branchId, productId, unitCode },
                select: { productId: true, branchId: true, unitCode: true, qtyOnHand: true },
            })
        )
    );
    const stockMap = new Map<string, number>();
    for (const s of currentStocks) {
        if (s) stockMap.set(`${s.productId}|${s.branchId}|${s.unitCode}`, Number(s.qtyOnHand));
    }

    // Walk movements oldest-first to compute running balance.
    // Running balance before movement[i] = currentStock - sum(qty of all later movements).
    // Since we fetch movements in DESC order, we reverse for computation.
    const sortedAsc = [...movements].reverse();
    // For each unique key, precompute total future qty and subtract as we go.
    const futureQtySum = new Map<string, number>();
    for (const m of sortedAsc) {
        const key = `${m.productId}|${m.branchId}|${m.unitCode}`;
        futureQtySum.set(key, (futureQtySum.get(key) || 0) + Number(m.qty));
    }

    const movementsWithRunning = movements.map((movement) => {
        const key = `${movement.productId}|${movement.branchId}|${movement.unitCode}`;
        const currentStock = stockMap.get(key) || 0;
        const sumFuture = futureQtySum.get(key) || 0;
        const remaining = sumFuture - Number(movement.qty);
        futureQtySum.set(key, remaining);
        const runningQty = currentStock - remaining;
...
inventoryRoutes.get('/transfers', requirePermission(PERMISSIONS.INVENTORY_VIEW as any), validate({ query: transferListQuerySchema }), asyncHandler(async (req, res) => {
...
inventoryRoutes.post('/transfers', requirePermission(PERMISSIONS.INVENTORY_TRANSFER as any), validate({ body: createTransferSchema }), asyncHandler(async (req, res) => {
...
inventoryRoutes.get('/transfers/:id', requirePermission(PERMISSIONS.INVENTORY_VIEW as any), validate({ params: idParamsSchema }), asyncHandler(async (req, res) => {
...
inventoryRoutes.post('/transfers/:id/send', requirePermission(PERMISSIONS.INVENTORY_TRANSFER as any), validate({ params: idParamsSchema }), asyncHandler(async (req, res) => {
    // @ts-ignore
    const transfer = await prisma.transfer.findFirst({
        where: {
            id: req.params.id as string,
    ...
    const sendResult = await prisma.$transaction(async (tx) => {
        ...
        // Deduct Stock from Source (batched)
        const transferOutMutations = (transfer as any).items.map((item: any) => ({
            companyId: req.user!.companyId,
        ...
        await InventoryService.mutateStockBatch(tx, transferOutMutations);
...
inventoryRoutes.post('/transfers/:id/receive', requirePermission(PERMISSIONS.INVENTORY_TRANSFER as any), validate({ params: idParamsSchema }), asyncHandler(async (req, res) => {
    // @ts-ignore
    const transfer = await prisma.transfer.findFirst({
        where: {
            id: req.params.id as string,
    ...
    const receiveResult = await prisma.$transaction(async (tx) => {
        ...
        // Add Stock to Destination (batched)
        const transferInItems = (transfer as any).items;
        const sourceMoves = await tx.stockMovement.findMany({
            where: {
                companyId: req.user!.companyId,
                referenceType: 'TRANSFER',
                referenceId: (transfer as any).id,
                type: 'TRANSFER_OUT',
                productId: { in: transferInItems.map((i: any) => i.productId) },
            },
            select: { productId: true, unitCode: true, cost: true },
            orderBy: { createdAt: 'desc' },
        });
        const sourceCostMap = new Map<string, number>();
        for (const sm of sourceMoves) {
            sourceCostMap.set(`${sm.productId}::${sm.unitCode}`, Number(sm.cost || 0));
        }
        const transferInMutations = transferInItems.map((item: any) => ({
            companyId: req.user!.companyId,
            branchId: (transfer as any).toBranchId,
            productId: item.productId,
            unitCode: item.unitCode,
            qtyChange: Number(item.qty),
            cost: sourceCostMap.get(`${item.productId}::${item.unitCode}`) || 0,
            type: 'TRANSFER_IN',
            referenceType: 'TRANSFER',
            referenceId: (transfer as any).id,
            createdById: req.user!.id,
        }));
        await InventoryService.mutateStockBatch(tx, transferInMutations);
...
inventoryRoutes.get('/stock-counts', requirePermission(PERMISSIONS.INVENTORY_VIEW), validate({ query: stockCountListQuerySchema }), asyncHandler(async (req, res) => {
...
inventoryRoutes.post('/stock-counts', requirePermission(PERMISSIONS.INVENTORY_AUDIT), validate({ body: stockCountSchema }), asyncHandler(async (req, res) => {
...
inventoryRoutes.put('/stock-counts/:id', requirePermission(PERMISSIONS.INVENTORY_AUDIT), validate({ params: idParamsSchema, body: stockCountSchema }), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { branchId, priceGroupId, notes, items } = req.body;
...
inventoryRoutes.get('/stock-counts/:id', requirePermission(PERMISSIONS.INVENTORY_VIEW), validate({ params: idParamsSchema }), asyncHandler(async (req, res) => {
...
inventoryRoutes.post('/stock-counts/:id/commit', requirePermission(PERMISSIONS.INVENTORY_AUDIT), validate({ params: idParamsSchema }), asyncHandler(async (req, res) => {
    ...
    const commitResult = await prisma.$transaction(async (tx) => {
        ...
        // Pre-filter items with non-zero variance and batch-determine costs
        const varyingItems = (sc as any).items.filter((item: any) => Number(item.variance) !== 0);
        const productIds = [...new Set(varyingItems.map((i: any) => i.productId as string))] as string[];

        // Bulk fetch current stock records
        const stockRecords = await tx.inventoryStock.findMany({
            ...
            select: { productId: true, unitCode: true, avgCost: true },
        });
        const stockCostMap = new Map<string, number>();
        for (const sr of stockRecords) {
            stockCostMap.set(`${sr.productId}::${sr.unitCode}`, Number(sr.avgCost || 0));
        }

        // Fallback: fetch products for items not found in stock
        const missingProductIds = varyingItems
            .filter((i: any) => !stockCostMap.has(`${i.productId}::${i.unitCode}`))
            .map((i: any) => i.productId as string);
        const unitCostMap = new Map<string, number>();
        if (missingProductIds.length > 0) {
            const products = await tx.product.findMany({
                where: { id: { in: missingProductIds } },
                include: { units: { select: { unitCode: true, costPrice: true } } },
            });
            for (const p of products) {
                for (const u of (p as any).units || []) {
                    unitCostMap.set(`${p.id}::${u.unitCode}`, Number(u.costPrice || 0));
                }
            }
        }

        const stockAdjustMutations = varyingItems.map((item: any) => {
            const variance = Number(item.variance);
            const cost = stockCostMap.get(`${item.productId}::${item.unitCode}`)
                ?? unitCostMap.get(`${item.productId}::${item.unitCode}`)
                ?? 0;
            return {
                companyId: req.user!.companyId,
                branchId: (sc as any).branchId,
        ...
        await InventoryService.mutateStockBatch(tx, stockAdjustMutations);

        // Bucket for Accounting double-entry
        for (const m of stockAdjustMutations) {
            if (m.qtyChange < 0) {
                losses.push({ productId: m.productId, qtyChange: m.qtyChange, cost: m.cost });
            } else {
                gains.push({ productId: m.productId, qtyChange: m.qtyChange, cost: m.cost });
            }
        }
...
inventoryRoutes.get('/analytics', requirePermission(PERMISSIONS.INVENTORY_VIEW), asyncHandler(async (req, res) => {
    const now = new Date();
